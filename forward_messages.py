...
MESSAGE_SINCE_DAYS = int(environ.get("MESSAGE_SINCE_DAYS"))
        ...
        for key, val in environ.items():
            if key.startswith("CONTACT_ID"):
                if not val.startswith("@"):
                    ...
                    CONTACT_CHAT_IDS.append(val)
            elif key.startswith("CHANNEL_ID"):
                CHANNEL_ID = await client.get_entity(PeerChannel(int(val)))
                print(CHANNEL_ID)

        now = datetime.now(timezone.utc)
        specified_hours = now - timedelta(hours=MESSAGE_SINCE_HR, days=MESSAGE_SINCE_DAYS)
